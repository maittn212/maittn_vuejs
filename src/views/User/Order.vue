...
const callAPI = async () => {
    try {
        ...
        let respone = await axios.get(urlOrder + "?user_id=" + user_id)
        if (respone.status == 200) {
            order.value = respone.data
        }
    } catch (error) {
        console.log(error);
...
const handleCancel = async (order_id) => {
    const check = confirm("Bạn có muốn hủy không?")
    if (check) {
        try {
            let respone = await axios.get(urlChangeStatus + "?order_id=" + order_id + "&status_code=3")
            // console.log(respone);
                ...
                <tbody>
                    <tr v-for="(item, index) in order" :key="index">
                        ...
                        <td>
                            ...
                            <span v-else-if="item.status == 4" class="badge text-bg-primary">Đang giao hàng</span>
                        ...
                        <td>
                            ...
                            <button v-if="item.status == 1" class="btn btn-danger" @click="handleCancel(item.order_id)">Hủy đơn</button>
                        </td>
                    </tr>
                </tbody>
            </table>
